...
export function validateWalkForwardWindowReference(historyInput, options = {}) {
  ...
  if (history.length < WINDOW_TARGETS + minTrain) {
    throw new Error(`Reference window membutuhkan minimal ${WINDOW_TARGETS + minTrain} draw pada slice ini.`);
  }
  ...
  for (const targetIndex of calibrationIndices) {
    ...
    if (training.length < minTrain) throw new Error("Reference training window terlalu pendek untuk calibration.");
  ...
  for (let targetIndex = 0; targetIndex < HOLDOUT_TARGETS; targetIndex += 1) {
    const target = history[targetIndex];
    const training = trainingSlice(history, targetIndex);
    if (training.length < minTrain) throw new Error("Reference training window terlalu pendek untuk holdout.");
    const arrays = rankArrays(training, rankingOptions);
  ...
  return {
    meta: {
      version: "0.6.4-reference",
      method: "frozen-reference-walk-forward-window",
      ...
      minTrain,
      note: "Frozen reference implementation untuk parity audit. Jangan dioptimasi tanpa membuat baseline reference baru.",
    },
    gate: {
      passed,
      status: passed ? "validated" : "locked",
      label: passed ? "WINDOW PASS" : "WINDOW LOCKED",
      reason: "Reference gate hanya untuk parity audit dan diagnostic comparison.",
    },
