...
function initGL(){
  ...
  const fs=`#version 300 es
    ...
    vec3 stars(vec2 u,float gain){vec2 g=u*vec2(120.,70.);vec2 id=floor(g);vec2 f=fract(g)-.5;float d=length(f-(vec2(h(id),h(id+13.7))-.5));float s=smoothstep(.035,0.,d)*step(.965,h(id+4.));return vec3(.55,.72,1.)*s*gain;}
    vec3 draw(float s,vec2 u){
      ...
      else if(s<4.5){vec2 pp=p-vec2(.22,-.02);float pr=length(pp);float body=smoothstep(.24,.225,pr);c=mix(c,vec3(.02,.28,.58)+vec3(.04,.35,.35)*max(0.,1.-pr*3.),body);float ring=exp(-abs(length(vec2(p.x,p.y*4.2))-.44)*90.);c+=vec3(.35,.72,1.)*ring*.34;c+=stars(u,.26);}
      else if(s<5.5){float rays=pow(.5+.5*cos(a*34.+t*1.8),9.)*smoothstep(.08,.8,r);c+=vec3(.18,.48,1.)*rays*(1.-r)*.8+c*0.;c+=stars(u,.18);}
      else if(s<6.5){float shell=exp(-abs(r-(.17+.012*sin(t*.8)))*35.);c+=mix(vec3(1.,.08,.02),vec3(1.,.82,.42),1.-r)*shell*1.2+c;c+=vec3(1.,.75,.45)*exp(-r*18.);}
      ...
      else if(s<9.5){c+=stars(u,.28);float corona=exp(-abs(r-.205)*42.);c+=vec3(1.,.55,.16)*corona*.9;c*=1.-smoothstep(.205,.19,r)*.97;}
    ...
    void main(){
      vec2 u=uv;vec2 m=(mouse-.5)*.018;u+=m;
...
function render(now){
  ...
  gl.uniform1f(u("scene"),current);
  gl.uniform1f(u("prevScene"),previous);
  gl.uniform1f(u("blend"),reduced?1:mixv);
...
function init(){
  ...
  canvas.addEventListener("webglcontextlost",e=>{e.preventDefault();cancelAnimationFrame(raf);fallback();},{once:true});
